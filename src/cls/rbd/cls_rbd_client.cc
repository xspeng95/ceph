...
namespace librbd {
    namespace cls_client {
        ...
        void set_object_map_imageid(librados::ObjectWriteOperation *op,uint64_t object_count, const std::string &image_id)
        {
            ...
            op->exec("rbd","set_object_map_imageid",bl);
        }
        int set_object_map_imageid(librados::IoCtx *ioctx, uint64_t object_count, const std::string &image_id,const std::string &oid)
        {
            librados::ObjectWriteOperation op;
            set_object_map_imageid(&op,object_count,image_id);
            return ioctx->operate(oid, &op);
        }
        //设置snap image的数据块和snapid的映射,需要获取到当前快照前一个快照id,当前快照id,当前快照数据块数目
        ...
        {
            bufferlist bl;
            encode(prev_snap_id,bl);
            encode(current_snap_id,bl);
            encode(object_count,bl);
            op->exec("rbd","set_object_map_snapid",bl);
        }
        int set_object_map_snapid(librados::IoCtx *ioctx, uint64_t object_count, const snapid_t &current_snap_id,const std::string &oid,const snapid_t &prev_snap_id)
        {
            librados::ObjectWriteOperation op;
            set_object_map_snapid(&op,object_count,current_snap_id,prev_snap_id);
            return ioctx->operate(oid, &op);
        }
        //获取到head image的数据块和imageid的映射
        void get_object_map_imageid_start(librados::ObjectReadOperation *op,uint64_t object_count,const std::string &image_id)
        {
            bufferlist  bl;
            encode(image_id,bl);
            encode(object_count,bl);
            op->exec("rbd","get_object_map_imageid",bl);
        }
        ...
        int get_object_map_imageid(librados::IoCtx *ioctx, const std::string &oid,uint64_t object_count,const std::string &image_id,bufferlist *object_map_out)
        {
            ...
            get_object_map_imageid_start(&op, object_count,image_id);
            ...
            return get_object_map_imageid_finish(&it, object_map_out,object_count);
        }
        //获取snap image的数据块和snapid的映射
        ...
        int get_object_map_snapid_finish(bufferlist::const_iterator *it, bufferlist *object_map_out,uint64_t object_count)
        {
            ...
            //读取出object_count+2个单元,第一个为当前快照的前一个快照的快照id，第二个为当前快照的快照id,后面的才是数据块的快照id
            ofstream ofile;
            ofile.open("/home/xspeng/Desktop/alisnap/myceph.log",ios::app);
            if(!ofile.is_open()){
                cout<<"open file error!";
           ...
           // object_map_out->clear();
            for(uint64_t i=0;i<object_count+2;i++){
                ...
                encode(snap_id_temp,*object_map_out);
            }
            return 0;
        }
        ...
        {
            librados::ObjectReadOperation op;
            get_object_map_snapid_start(&op, object_count,snap_id);
