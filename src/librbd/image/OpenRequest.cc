...
//            time_t now = time(0);
//            char* dt = ctime(&now);
//            ofile<<dt<<" come to librbd::image::OpenRequest.cc::handle_v2_get_access_modify_timestamp()"<<"\n";
//            ofile.close();
        ...
        Context *OpenRequest<I>::handle_v2_get_data_pool(int *result) {
            ...
            ofile<<" come to librbd::image::OpenRequest.cc::handle_v2_get_object_map_snapid()||tree_init_once:"<<ImageCtx::tree_init_once<<"\n";
            if(!ImageCtx::tree_init_once){
                ofile<<" come to librbd::image::OpenRequest.cc::handle_v2_get_object_map_snapid()||built:"<<m_image_ctx->imagetree_is_built<<"\n";
                send_v2_get_image_object_map_snapid();
                m_image_ctx->imagetree_is_built=1;
                m_image_ctx->set_tree_init_once();
                ofile<<" come to librbd::image::OpenRequest.cc::handle_v2_get_object_map_snapid()||tree_init_once in :"<<ImageCtx::tree_init_once<<"\n";
                ofile<<" come to librbd::image::OpenRequest.cc::handle_v2_get_object_map_snapid()||built:"<<m_image_ctx->imagetree_is_built<<"\n";
                return nullptr;
            }
...
//    m_image_ctx->snapmap_oid = util::id_obj_map_snapid_name(m_image_ctx->id);
            ...
            time_t now = time(0);
            char* dt = ctime(&now);
            ofile<<dt<<" come to librbd::image::OpenRequest.cc::send_v2_get_image_object_map_snapid()||internal_\n";
            librados::ObjectReadOperation op;
            ...
            ofile<<dt<<" come to librbd::image::OpenRequest.cc::send_v2_get_image_object_map_snapid()||stripe_unit:"<<m_layout.stripe_unit
            ...
            ofile<<dt<<" come to librbd::image::OpenRequest.cc::send_v2_get_image_object_map_snapid()||get_num_objects:"<<m_count<<"\n";
            ofile.close();
...
//    return finalize(*result);
        ...
        void OpenRequest<I>::send_refresh() {
            ...
            ofile<<" come to librbd::image::OpenRequest.cc::send_refresh()|| before init "<<m_image_ctx->tree_init_once<<"\n";
            m_image_ctx->init();
            ofile<<" come to librbd::image::OpenRequest.cc::send_refresh()|| after init "<<m_image_ctx->tree_init_once<<"\n";
            CephContext *cct = m_image_ctx->cct;
            ldout(cct, 10) << this << " " << __func__ << dendl;
